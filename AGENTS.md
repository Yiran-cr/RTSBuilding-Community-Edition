...
| `power` | **电网核心逻辑**（参照 power_system_design.md；**电网=物理链路连通图**，先按归属组管辖、组内再按「链路连接 + 塔覆盖接入」物理分区；**覆盖融合统一供电**）：`PowerRole`（GENERATOR/TOWER 节点角色）、`GridNode`（调度输入快照，坐标用 long 解耦 MC）、`PowerScheduler`（纯算法：union-find 划物理电网——链路阈值=两端 linkRange 取最大值、**塔供电范围覆盖发电机即接入**、发电-发电不直连、断开即独立电网——并输出每组 `GridAllocation(totalGeneration, groupIndices)`；发电量=组网发电机 `generation()` + 各塔上报的外部提取 `externalGeneration()`）、`IPowerGridNode`（方块实体实现的节点接口：role/linkRange/powerRange/throughput/generation/demand/gridOwner + 组级广播接口 `coveredConsumers()/consumerRoom()/injectTo()/externalGeneration()`）、`PowerGridManager`（服务端每维度调度器：按归属组分组、每 tick 聚合发电 + **组级统一广播 `broadcastToGrid`——电网内所有塔覆盖区融合，按用电器需求占比统一注入、覆盖重叠去重、忽略单塔吞吐**、nodesOf/aggregate 聚合查询，委托全局所有权；物理隔离节点不共享）、`PowerGridOwnership`（服务端**全局电网所有权**：参照 Flux-Networks changeMembership/NetworkMember/AccessLevel——owner/members 带 RtsAccessLevel(OWNER/ADMIN/USER) 阶梯、changeMembership(actor,target,action,online)→结果码(SUCCESS/NO_ADMIN/NO_OWNER/INVALID)、邀请只对在线玩家、升降级/移除/转移所有权（旧主降 USER、目标在线且为成员、不能转给自己）、组持久化 `config/rts_building/powergrid_groups.json` 重启保留，成员跨维度共享） |
...
- **电网供电链路（参照 power_system_design.md）**：发电机器（发电机/风电机）`tickServer` 上报本 tick 产电速率 `generation()`；输电塔 `tickServer` 分片扫描其<b>供电范围</b>发现用电器（`consumers`），并从供电范围<b>外部发电机</b>提取电量（`externalGeneration()`，塔本机不产电）→ 二者向 `PowerGridManager` 注册；`PowerGridManager.tick` 每 tick 用 `PowerScheduler.allocateByGrid` 按<b>电网组</b>（先按归属组管辖、组内再按「链路连接 + 塔覆盖接入」union-find 物理分区——链路阈值=两端 linkRange 取最大值、塔供电范围覆盖发电机即接入、发电-发电不直连、断开即独立电网）聚合发电量 → **组级统一广播 `broadcastToGrid`**：电网内所有塔的<b>供电范围融合</b>成联合覆盖区，按各用电器<b>需求占比</b>统一注入（覆盖重叠去重，忽略单塔吞吐，转发不衰减），每个用电器由覆盖它的任一塔执行 `injectTo`。节点归属：放置者经 `RtsGridOwnerBinding`（common 桥）绑定 `gridOwner`。**电网多人**：`PowerGridOwnership` 全局组（唯一所有者 + 成员邀请/移除，UI 按 K 打开 `PowerGridManagerPanel` 管理），组快照经 S2C `S2CPowerGridSnapshot` 推送到客户端缓存，main UI 经 `RtsPowerGrid`（api）读取。外部模组经方块 `IEnergyStorage` capability ↔ `ContainerEnergyStorage` 适配器互动；塔/发电机自带 `BasicEnergyContainer` 仅作外部接口查看、不参与电网能量搬运。建造操作不耗能。
